...
export default function ScanQRCode({ onScanned }) {
  ...
  const handleScan = async (detectedCodes) => {
    ...
    // Take the first scanned QR code
    const qrValue = detectedCodes[0].rawValue;

    try {
      const res = await scanQRCode(qrValue);
      onScanned(res.data);
      navigate(`/patient/${res.data.patientId}`, { state: { patient: res.data } });

    } catch (err) {
      alert("Patient not found — try registering temporary.");
    } finally {
